...
class View:
    def __init__(self, top=None):
        ...
        self.songName.configure(text='''Play music''')
        ...
        self._img4 = tk.PhotoImage(file="./icons/music4.png")
    ...
    def set_player(self):
        try:
            ...
            if self.my_player.get_db_status():
                messagebox.showinfo(("Success!", "connected successfully to DB!!!"))
            else:
        ...
        self.Button10.configure(command=self.remove_song_favourites)
        ...
        img = tk.PhotoImage(file="./icons/titleicon.png")
        self.top.iconphoto(self.top, img)
        self.top.title("MOZIKKA - Dance to the rythm of your heart!!" )
    ...
    def close_window(self):
        result=messagebox.askyesno("MOZIKKA","Do you want to quit?")
        if(result):
            self.my_player.close_player()
            messagebox.showinfo("Have a good day!","Thank you for using \"Mozikka\"")
            self.top.destroy()
    ...
    def load_previous_song(self):
        try:
            if hasattr(self, "sel_song_index_tuple") == False:
                raise NoSongSelectedError("Please select a first song")
    ...
    def remove_song_favourites(self):
        fav_song_index_tuple = self.playList.curselection()
        try:
            if len(fav_song_index_tuple) == 0:
                raise NoSongSelectedError("Please select a song to add to favourites")
